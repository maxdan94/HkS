...
to compile:
gcc improvestochastic.c -o improvestochastic -O9
...
double positifswap(sparse* g, subgraph* sg, unsigned *bi, unsigned *bj){
	unsigned i,j;
	unsigned n1,n2;
...
void optimize(sparse *g,subgraph *sg){
	...
	unsigned i,j,k=0;
	while (1){
		randomize(sg);//randomize the order of the nodes
		delta=positifswap(g,sg,&i,&j);
