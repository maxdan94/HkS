...
Info:
Feel free to use these lines as you wish. This program finds the densest subgraph of k nodes. The reusult is exact. The algorithm is based on a smart branch and bound: the branching phase is based on deciding whether to add a specific edge (and thus its endpoints) in the corresponding solution or not.
...
treenode* find_dks(bheap* heap,edgelist* el, graph* g, unsigned k){
...
//Printing results
void printres(treenode* trn,edgelist *el) {
	unsigned i=0;
	printf("val: %.10le\n",trn->val);
	printf("size: %u\n",trn->size);
	printf("nodes:");
